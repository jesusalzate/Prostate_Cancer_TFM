...
class SwinUnetr_multitask_post(nn.Module):
    """
    ...
    def __init__(
        ...
        """
        super(SwinUnetr_multitask_post, self).__init__()
    ...
    def forward(self, x_in):
        ...
        class_out = F.softmax(self.out_class(x1),dim=1)
        # Segmentation task
        ...
        # Apply softmax to the segmentation logits
        logits_seg = F.softmax(logits_seg, dim=1)

        with torch.no_grad():
            # Apply Report Guided Annotation
            # Extract lesion candidates from the segmentation map.
            class_scores = torch.empty(logits_seg.shape[0])
            for i, (logit, class_value) in enumerate(zip(logits_seg, class_out)):
                detection_map = extract_lesion_candidates(logit[1].cpu().numpy(), threshold="dynamic")[0]
                print(class_value)
                class_scores[i] = (np.max(detection_map) + class_value[1].item()) / 2.

            class_out = torch.unsqueeze(class_scores, dim=1)
